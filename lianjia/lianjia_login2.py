...
# 单地址采集配置（仅采集松江）
TARGET_CITY_NAME = "上海"
TARGET_DISTRICT_NAME = "松江"
TARGET_DISTRICT_URL = "https://sh.lianjia.com/zufang/songjiang/"
START_PAGE = 52  # 从第几页开始继续爬取
MAX_PAGES_PER_DISTRICT = 200  # 每个区域最大爬取页数（上限保护）
MAX_TOTAL_DATA = 1500  # 总目标数据量（条）
...
def save_login_state():
    ...
    initial_url = TARGET_DISTRICT_URL.rstrip("/") + "/"
    print(f"[配置] 单地址模式: {TARGET_CITY_NAME}-{TARGET_DISTRICT_NAME}")
    print(f"[配置] 登录入口页面: {initial_url}")
...
def crawl_multiple_districts():
    ...
    target_url = TARGET_DISTRICT_URL.rstrip("/") + "/"
    parsed_target = urlsplit(target_url)
    if not parsed_target.scheme or not parsed_target.netloc:
        print(f"[错误] 目标地址配置不合法: {TARGET_DISTRICT_URL}")
        return

    CITY_NAME = TARGET_CITY_NAME
    CITY_DOMAIN = parsed_target.netloc
    districts = [{"name": TARGET_DISTRICT_NAME, "url": target_url}]
    print(f"[配置] 已启用单区域模式: {TARGET_DISTRICT_NAME} ({target_url})")

    # 加载已存在数据用于去重
    existing_data = load_existing_data()
    existing_count = len(existing_data)
    if existing_count >= MAX_TOTAL_DATA:
        print(f"[完成] 已有数据 {existing_count} 条，已达到目标 {MAX_TOTAL_DATA} 条")
        return

    print(f"\n" + "=" * 60)
    print(f"[开始] 准备爬取 {CITY_NAME} 的租房数据")
    print(f"[配置] 目标数据量: {MAX_TOTAL_DATA} 条 (当前已有 {existing_count} 条)")
    print(f"[配置] 每个区域最多: {MAX_PAGES_PER_DISTRICT} 页")
    print(f"[配置] 从第 {START_PAGE} 页开始继续爬取")
    ...
    with sync_playwright() as p:
        ...
        for district_idx, district in enumerate(districts, 1):
            ...
            if START_PAGE > 1:
                start_url = f"{district_url.rstrip('/')}/pg{START_PAGE}/#contentList"
            if not goto_with_retry(page, start_url, max_retry=2, retry_delay=RETRY_DELAY):
            ...
            for page_num in range(START_PAGE, MAX_PAGES_PER_DISTRICT + 1):
                # 再次检查数据量
                if existing_count + total_new_data >= MAX_TOTAL_DATA:
                ...
                if rental_data:
                    ...
                    print(f"[区域] 该区域累计: {district_data_count} 条，总计: {existing_count + total_new_data} 条")
